...
(function () {
    ...
    const SHOW_PWD = document.getElementById("showPwdEye");
    const SHOW_REPEAT_PWD = document.getElementById("showRepeatEye");

    const SHOW_PWD_PATH = document.getElementById("showPwdIconPath");
    const SHOW_REPEAT_PATH = document.getElementById("showRepeatIconPath");

    const SHOW_PWD_ICON_PATH_DATA = SHOW_PWD_PATH.getAttribute("d");
    ...
    /* Toggling show password eye */
    let showPassword = true;
    let showRepeat = true;

    function togglePassword(repeatPassword) {
        let pathToChange = SHOW_PWD_PATH;
        let fieldToChange = PASSWORD;

        function updateEye(conditional) {
            if (conditional) {
                pathToChange.setAttribute("d", SHOW_PWD_ICON_PATH_DATA);
                fieldToChange.type = "password";
            } else {
                pathToChange.setAttribute("d", HIDE_PWD_ICON_SVG_DATA);
                fieldToChange.type = "text";
            }
        }

        if (repeatPassword) {
            pathToChange = SHOW_REPEAT_PATH;
            fieldToChange = REPEATED_PASSWORD;
            showRepeat = !showRepeat;

            updateEye(showRepeat);
            return;
        }

        showPassword = !showPassword;
        updateEye(showPassword);
    }

    SHOW_PWD.addEventListener("click", () => {
        togglePassword(false);
    });
    SHOW_REPEAT_PWD.addEventListener("click", () => {
        togglePassword(true);
    });

    /* Handle submit of form and register user if fields are valid */
    REGISTER_FORM.addEventListener("submit", async (e) => {
        ...
        const REPEAT_FEEDBACK = document.getElementById("repeatFeedback");
        const GENERAL_FEEDBACK = document.getElementById("generalFeedback");
        const VALID_EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/i;
        const VALID_USERNAME_REGEX = /^[a-zA-Z0-9_]{3,15}$/;
        ...
        if (!EMAIL.value || !VALID_EMAIL_REGEX.test(EMAIL.value)) {
            ...
            SHOW_PWD.classList.remove("invalid");
            SHOW_REPEAT_PWD.classList.remove("invalid");
            return;
        } else {
        ...
        if (!USERNAME || !VALID_USERNAME_REGEX.test(USERNAME.value)) {
        ...
        if (!PASSWORD || PASSWORD.value.length < 8) {
            ...
            SHOW_PWD.classList.add("invalid");
        ...
        if (REPEATED_PASSWORD.value !== PASSWORD.value) {
            ...
            SHOW_REPEAT_PWD.classList.add("invalid");
            REPEAT_FEEDBACK.style.display = "block";
            return;
        } else {
            SHOW_REPEAT_PWD.classList.remove("invalid");
            REPEAT_FEEDBACK.style.display = "none";
        ...
        try {
            const response = await fetch(`${SERVER_PATH}users/`, options);
